...
func main() {
	...
	testResults["control_transfers"] = testControlTransfers(ctx, devices)
	
	// Test 3: Interface management
	fmt.Println("\n🔧 Test 3: Interface Management")
	testResults["interface_mgmt"] = testInterfaceManagement(ctx, devices)
	
	// Test 4: Async transfer system
	fmt.Println("\n⚡ Test 4: Async Transfer System")
	testResults["async_transfers"] = testAsyncTransfers(ctx, devices)
	
	// Test 5: Event handling
	fmt.Println("\n⏱️  Test 5: Event Handling")
	testResults["event_handling"] = testEventHandling(ctx)
...
func testControlTransfers(ctx *usb.Context, devices []*usb.Device) bool {
	successCount := 0
	totalAttempts := 0
...
func testInterfaceManagement(ctx *usb.Context, devices []*usb.Device) bool {
	successCount := 0
	totalAttempts := 0
...
func testAsyncTransfers(ctx *usb.Context, devices []*usb.Device) bool {
	successCount := 0
	totalAttempts := 0
	...
	for i, dev := range devices {
		...
		if err != nil {
			handle.Close()
			fmt.Printf("   Device %d: ❌ Could not create transfer manager: %v\n", i, err)
			continue
		}
		
		// Test transfer creation
		transfer := manager.NewAsyncTransfer(0x80, usb.TransferTypeBulk, 64, 0)
		if transfer == nil {
			manager.Close()
			handle.Close()
			fmt.Printf("   Device %d: ❌ Could not create transfer\n", i)
			continue
		}
		
		// Test transfer configuration
		callbackCalled := false
		transfer.SetCallback(func(t *usb.AsyncTransfer) {
			callbackCalled = true
		})
		_ = callbackCalled // Suppress unused variable warning
		transfer.SetTimeout(500 * time.Millisecond)
		transfer.SetUserData("test")
		
		// Test transfer properties
		if status := transfer.GetStatus(); status != usb.TransferCompleted {
			manager.Close()
			handle.Close()
			fmt.Printf("   Device %d: ❌ Unexpected initial status: %v\n", i, status)
			continue
		}
		
		// Test isochronous transfer creation
		isoTransfer := manager.NewAsyncTransfer(0x81, usb.TransferTypeIsochronous, 512, 4)
		if isoTransfer != nil {
			packets := isoTransfer.GetIsoPackets()
			if len(packets) != 4 {
				fmt.Printf("   Device %d: ⚠️  Isochronous packets: expected 4, got %d\n", i, len(packets))
			}
		}
		...
		err = manager.Start()
		if err == nil {
			fmt.Printf("   Device %d: ✅ Async transfer system operational\n", i)
			successCount++
		} else {
			fmt.Printf("   Device %d: ❌ Manager start failed: %v\n", i, err)
		}
...
func testEventHandling(ctx *usb.Context) bool {
